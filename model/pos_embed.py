...
def build_abs_pos_embedding(scheme: str, hidden_dim: int) -> Union[nn.Module, None]:
    if scheme == 'learned':
        return LearnedEmbedding(hidden_dim)
    elif scheme == 'sinusoidal':
        return SinusoidalEmbedding(hidden_dim)
    elif scheme == 'no':
        return None

    raise ValueError(f'Not supported positional embedding type {scheme}')