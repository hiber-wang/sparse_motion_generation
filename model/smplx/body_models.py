## This is code is modified from the official implementation of SMPL-X
...
def _expand_batch(t, bs=1):
    return t.expand((bs, *(-1 for _ in t.shape[1:]))) if t.shape[0] != bs else t
...
class SMPL(nn.Module):
	...
	kid_template_path: str = '',
        ...
        **kwargs
        ...
        if create_body_pose:
            if body_pose is None:
                default_body_pose = torch.zeros(
                    [1, self.NUM_BODY_JOINTS * 3], dtype=dtype)
            else:
        ...
        if create_transl:
            if transl is None:
                default_transl = torch.zeros([1, 3],
                                             dtype=dtype,
                                             requires_grad=True)
            else:
    ...
    def forward(
        ...
        **kwargs
        ...
        ''' Forward pass for the SMPL model
            ...
            -------
        '''
        batch_size = _get_batch_size((betas, global_orient, body_pose, transl))
        ...
        betas, global_orient, body_pose, transl = map(partial(_expand_batch, bs=batch_size), (betas, global_orient, body_pose, transl))
...
class SMPLH(SMPL):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(SMPLH, self).__init__(
            ...
            vertex_ids=vertex_ids, gender=gender, age=age,
        ...
        if create_left_hand_pose:
            if left_hand_pose is None:
                default_lhand_pose = torch.zeros([1, hand_pose_dim],
                                                 dtype=dtype)
            else:
        ...
        if create_right_hand_pose:
            if right_hand_pose is None:
                default_rhand_pose = torch.zeros([1, hand_pose_dim],
                                                 dtype=dtype)
            else:
    ...
    def forward(
        ...
        **kwargs
        ...
        '''
        '''
        batch_size = _get_batch_size((betas, global_orient, body_pose, left_hand_pose, right_hand_pose, transl))
        # If no shape and pose parameters are passed along, then use the
        # ones from the module
        global_orient = (global_orient if global_orient is not None else
                         self.global_orient)
        ...
        betas, global_orient, body_pose, left_hand_pose, right_hand_pose, transl = map(partial(_expand_batch, bs=batch_size), betas, global_orient, body_pose, left_hand_pose, right_hand_pose, transl)
...
class SMPLX(SMPLH):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if create_jaw_pose:
            if jaw_pose is None:
                default_jaw_pose = torch.zeros([1, 3], dtype=dtype)
            else:
        ...
        if create_leye_pose:
            if leye_pose is None:
                default_leye_pose = torch.zeros([1, 3], dtype=dtype)
            else:
        ...
        if create_reye_pose:
            if reye_pose is None:
                default_reye_pose = torch.zeros([1, 3], dtype=dtype)
            else:
        ...
        if create_expression:
            if expression is None:
                default_expression = torch.zeros(
                    [1, self.num_expression_coeffs], dtype=dtype)
            else:
        ...
        self.has_expression = create_expression
    ...
    def forward(
        ...
        **kwargs
        ...
        '''
        batch_size = _get_batch_size((betas, global_orient, body_pose, left_hand_pose, right_hand_pose, jaw_pose, leye_pose, reye_pose, expression, transl))
        # If no shape and pose parameters are passed along, then use the
        # ones from the module
        global_orient = (global_orient if global_orient is not None else
                         self.global_orient)
        ...
        if self.has_expression:
            ...
            expression = _expand_batch(expression, bs=batch_size)
        ...
        betas, global_orient, body_pose, left_hand_pose, right_hand_pose, jaw_pose, leye_pose, reye_pose, transl = map(
            partial(_expand_batch, bs=batch_size), (betas, global_orient, body_pose, left_hand_pose, right_hand_pose, jaw_pose, leye_pose, reye_pose, transl)
        )
